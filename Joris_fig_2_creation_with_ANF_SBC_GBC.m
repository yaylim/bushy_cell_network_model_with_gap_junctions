...
seed_number_vec = 1000:5:1045;
g_gap_vec = [0,5,10,20,50,100];
g_gap_strength_vec = [25,33,41,57,94,124]*3;
inh_vec = [0,5,10,20,50,100];
seed_number = 1000;

g_gap_index = 1; % the index of the gap junction strength level
inh_index = 1; % the index of the inhibition level
seed_number = 1000;
mult_exct_gbc = 0.75;
db_level = 50; % in dB
...
load (['Data/gap_junct_network_gbc_layer_out_340_g_gap_' num2str(g_gap_vec(g_gap_index)) '_inh_' num2str(inh_vec(inh_index)) '_exct_' num2str(g_gap_strength_vec(g_gap_index) * mult_exct_gbc/3) '_high_SR.mat'])
GBC_stored = BS_stored;
SI_GBC = SI;
fire_rate_sec_GBC = fire_rate_sec;
load (['Data/gap_junct_network_bs_layer_out_340_g_gap_' num2str(g_gap_vec(g_gap_index)) '_inh_' num2str(inh_vec(inh_index)) '_exct_' num2str(g_gap_strength_vec(g_gap_index)) 'high_SR_seed_' num2str(seed_number) '_v2.mat'])
...
yyaxis left
plot(db_vec_ticks, fire_rate_sec,'*'), ylim([0 400]),ylabel('Firing rate/sec')
hold on, plot(db_vec_ticks,fire_rate_sec)
yyaxis right
plot(db_vec_ticks,SI,'o'),ylabel('Synchrony'),hold on, plot(db_vec_ticks,SI,'--'),ylim([0 1])
xticks([-60 -40 0 50 100])
xticklabels({'-inf', '-40', '0', '50', '100'})
xlim([-60 120])
...
yyaxis left
plot(db_vec_ticks, fire_rate_sec_GBC,'*'), ylim([0 400]),ylabel('Firing rate/sec')
hold on, plot(db_vec_ticks,fire_rate_sec_GBC)
yyaxis right
plot(db_vec_ticks,SI_GBC,'o'),ylabel('Synchrony'),hold on, plot(db_vec_ticks,SI_GBC,'--'),ylim([0 1])
xticks([-60 -40 0 50 100])
xticklabels({'-inf', '-40', '0', '50', '100'})
xlim([-60 120])
xlabel('dB SPL')
...
title (t_layout,['ANF, SBC and GBC CF = ' num2str(bushy_CF) 'Hz, g\_gap = ' num2str(g_gap_vec(g_gap_index)), 'nS, g\_inh =  ' num2str(inh_vec(inh_index)) 'nS'])
